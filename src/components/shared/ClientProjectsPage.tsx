...
const ClientProjectsPage: React.FC<ClientProjectsPageProps> = ({
  initialProjects,
  totalPages,
}) => {
  ...
  const handleNextPage = async () => {
    if (currentPage < totalPages && !loading) {
      ...
      try {
        const nextProjectsData = await getAllProjects({
          pageNo: nextPage,
          limit: projectsPerPage,
        });
        setProjects(nextProjectsData.docs);
  ...
  const handlePreviousPage = async () => {
    if (currentPage > 1 && !loading) {
      ...
      try {
        const prevProjectsData = await getAllProjects({
          pageNo: prevPage,
          limit: projectsPerPage,
        });
        setProjects(prevProjectsData.docs);
        ...
        >
          {loading
            ...
            : projects.map((project, idx) => (
                <ProjectCard key={project.id} idx={idx} project={project} />
              ))}
        </motion.div>
      </AnimatePresence>
